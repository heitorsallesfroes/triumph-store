...
export default function SalesHistory() {
  ...
  useEffect(() => {
    filterSales();
  }, [statusFilter, searchTerm, debouncedProductFilter, dateFilter]);
  ...
  const filterSales = async () => {
    ...
    try {
      ...
      if (dateFilter.start) {
        query = query.gte('sale_date', dateFilter.start);
      }
      ...
      if (debouncedProductFilter) {
        ...
        const { data: saleItems } = await supabase
          ...
          .in('sale_id', filtered.map(s => s.id));
        ...
        if (saleItems) {
          ...
          if (products) {
            ...
            for (const item of saleItems) {
              ...
              if (productName && productName.includes(productSearchTerm)) {
                matchingSaleIds.add(item.sale_id);
              }
            }
          }
        }
    ...
    try {
      ...
      const items = await Promise.all(
        (saleItems || []).map(async (item) => {
          ...
          return {
            name: product ? `${product.model} ${product.color}` : 'Produto',
      ...
      const { data: accessories } = await supabase
        ...
        .select('quantity, cost, accessory_id, custom_name')
        .eq('sale_id', sale.id);
  ...
  if (loading) {
    return (
      <div className="p-8">
        <div className="text-white text-center">Carregando histórico de vendas...</div>
      </div>
    );
  }

  const filteredByStatus = statusFilter === 'all' ? sales : sales.filter(s => s.status === statusFilter);
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-gray-700 text-white rounded-lg px-10 py-2.5 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-gray-700 text-white rounded-lg px-4 py-2.5 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-gray-700 text-white rounded-lg pl-10 pr-4 py-2.5 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-gray-400">
          {filterLoading ? (
            ...
            <>
              Mostrando <span className="text-white font-semibold">{filteredSales.length}</span> vendas
              {(statusFilter !== 'all' || searchTerm || debouncedProductFilter || dateFilter.start || dateFilter.end) && ' (filtrado)'}
            </>
          )}
        </p>
      </div>
              ...
              >
                <div className="grid grid-cols-12 gap-4 items-start">
                      ...
                      <div>
                        ...
                        {sale.main_product && (
                          <p className="text-gray-300 text-sm mb-1">
                            {sale.main_product}
                            {sale.additional_items > 0 && (
                              <span className="text-gray-500"> +{sale.additional_items} {sale.additional_items === 1 ? 'item' : 'itens'}</span>
                            )}
                          </p>
                        )}
                  ...
                  <div className="col-span-12 lg:col-span-2 flex gap-2">
                    <button
                      onClick={() => {
                        console.log('SalesHistory: Opening edit for sale ID:', sale.id);
                        setEditingSaleId(sale.id);
                      }}
                      className="bg-orange-600 hover:bg-orange-700 text-white px-3 py-2 rounded-lg transition-colors flex items-center justify-center gap-2 flex-1"
                      title="Editar Venda"
                    >
                      <Edit size={16} />
                    ...
                    <button
                      onClick={() => {
                        console.log('SalesHistory: Opening receipt for sale');
                        console.log('SalesHistory: SALE OBJECT:', sale);
                        setSelectedSale(sale);
                      }}
                      className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg transition-colors flex items-center justify-center gap-2 flex-1"
                      title="Gerar Recibo"
                    >
                      <FileText size={16} />
                ...
                {/* Correios Shipping Label Section */}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {sale.tracking_code && (
                            <p className="text-xs text-gray-400">Rastreio: {sale.tracking_code}</p>
                          )}
                          {sale.shipping_status && (
                            <p className="text-xs text-green-500">{sale.shipping_status}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        {!sale.shipping_label_url ? (
                          <button
                            onClick={() => handleGenerateShippingLabel(sale)}
                            disabled={generatingLabel === sale.id}
                            className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg transition-colors flex items-center gap-2 text-sm font-semibold"
                          >
                          ...
                          <button
                            onClick={() => window.open(sale.shipping_label_url, '_blank')}
      ...
      {/* Edit Modal */}
      {editingSaleId && (
        <EditSale saleId={editingSaleId} onClose={() => {
